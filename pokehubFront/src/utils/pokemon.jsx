...
export const RAREZA_COLORS = {
  ...
  epico:      { bg: "rgba(123,45,139,0.22)",  text: "#ce93d8", border: "rgba(123,45,139,0.5)" },
  ...
  mitico:     { bg: "rgba(226,88,34,0.22)",   text: "#ff7043", border: "rgba(226,88,34,0.5)"  },
};
...
export function TipoBadge({ tipo }) {
  ...
  return (
    <span
      className="inline-block text-[0.55rem] tracking-[0.04em] px-[0.75rem] py-[0.3rem] rounded-full text-white capitalize [text-shadow:0_1px_2px_rgba(0,0,0,0.5)]"
...
export function RarezaBadge({ rareza }) {
  const key = rareza?.toLowerCase() ?? "comun";
  const r = RAREZA_COLORS[key] ?? RAREZA_COLORS.comun;
  return (
    <span
      className="inline-block text-[0.48rem] tracking-[0.05em] px-[0.65rem] py-[0.22rem] rounded-full border font-semibold capitalize"
